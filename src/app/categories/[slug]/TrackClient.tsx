...
export default function TrackClient({ track }: { track: ClientTrack }) {
  ...
  const [mystery, setMystery] = useState<boolean>(() => {
    ...
    return window.localStorage.getItem("gaming:mystery") === "1";
  });

  useEffect(() => {
    if (closed && mystery) setMystery(false);
  }, [closed, mystery]);

  useEffect(() => {
    if (typeof window !== "undefined") {
      window.localStorage.setItem("gaming:mystery", mystery ? "1" : "0");
    }
  }, [mystery]);
 ...
 const tabs = useMemo(() => {
  const base: { key: string; label: string; icon: any }[] = [
    { key: "overview", label: "Overview", icon: Info },
  ];

  if (isQuiz) {
    base.push(
      { key: "teamelig", label: "Team & Eligibility", icon: ListChecks },
      { key: "format", label: "Competition Format", icon: FileText },
      { key: "participation", label: "Participation", icon: ListChecks },
      { key: "rules", label: "Rules & Guidelines", icon: ShieldCheck },
  ...
  if (isGaming) {
    base.push(
      { key: "modes", label: "Game Titles & Modes", icon: Package },
      { key: "rules", label: "Disqualification Rules", icon: ShieldCheck },
      { key: "judging", label: "Judging & Scoring", icon: Scale },
    );
    if (Array.isArray(t.eligibility) && t.eligibility.length) {
      base.push({ key: "eligibility", label: "Eligibility", icon: FileText });
    }
    base.push({ key: "registration", label: "Registration", icon: FileText });
    return base;
  }

  base.push(
    { key: "what", label: "What to Build", icon: Package },
    { key: "deliverables", label: "What to Submit", icon: ListChecks },
    { key: "rules", label: "Rules & Regulations", icon: ShieldCheck },
    { key: "judging", label: "Judging", icon: Scale },
  );
  const includeEligibilityTab =
    Array.isArray(t.eligibility) && t.eligibility.length > 0;
  return includeEligibilityTab
    ? [...base, { key: "eligibility", label: "Eligibility", icon: FileText }]
    : base;
}, [isQuiz, isGaming, t.eligibility]);
  ...
  const penaltyBullets = useMemo(() => rulesArr.filter((r) => /(copyright|offensive|violent|inappropriate|external help|cheat|ai)/i.test(r)), [rulesArr]);
  ...
  const conductBullets = useMemo(() => rulesArr.filter((r) => /(original|plagiarism|nsfw|hate|ai|cheat|hack|exploit|offensive|abusive|misconduct|referee)/i.test(r)), [rulesArr]);
  const submissionBullets = useMemo(() => rulesArr.filter((r) => /(deadline|late|submit|submission|hand ?over|register|registration|form)/i.test(r)), [rulesArr]);

  const webGroups = useMemo(
    () => [
      { key: "disq", title: "Disqualification Rules", icon: "shield" as const, bullets: ["Using pre-made templates or copied code leads to immediate disqualification.", "Plagiarism or unmodified AI-generated code is not allowed.", "All work must be original and authentic."] },
      { key: "submit", title: "Submission Process", icon: "wrench" as const, bullets: ["Submit your project before the deadline.", "Include a 2–10 minute video showing website features & functionality.", "Explain theme alignment (Space Elevator).", "Demonstrate the user experience.", "Upload the video to YouTube/Google Drive and share the link along with project files."] },
    ],
    []
  );

  const videoThemeBullets = useMemo(() => rulesArr.filter((r) => /space elevator|theme/i.test(r)), [rulesArr]);
  const videoPenaltyBullets = useMemo(() => rulesArr.filter((r) => /(copyright|offensive|violent|inappropriate|cheat|external help)/i.test(r)), [rulesArr]);
  const videoSubmissionBullets = useMemo(() => rulesArr.filter((r) => /(submit|deadline|form)/i.test(r)), [rulesArr]);

  const videoGroups = useMemo(
    () => [
      { key: "theme", title: "Theme", icon: "lightbulb" as const, bullets: videoThemeBullets.length ? videoThemeBullets : ['The content should align with the theme "Space Elevator".'] },
      { key: "submit", title: "Submission Process", icon: "wrench" as const, bullets: videoSubmissionBullets.length ? videoSubmissionBullets : ["Submit your entry via the official form before the deadline."] },
      { key: "penalty", title: "Disqualification & Penalties", icon: "shield" as const, bullets: videoPenaltyBullets.length ? videoPenaltyBullets : ["Use of copyrighted materials without permission.", "Offensive, violent, or inappropriate content.", "Any form of cheating or external help."] },
    ],
  ...
  const defaultGroups = useMemo(
    () => [
      { key: "conduct", title: "Conduct & Originality", icon: "lightbulb" as const, bullets: conductBullets.length ? conductBullets : ["Originality and fair play are required."] },
      { key: "submission", title: "Submission Process", icon: "wrench" as const, bullets: submissionBullets.length ? submissionBullets : ["All entries must be submitted before the deadline.", "Late submissions will not be accepted."] },
    ],
  ...
  const quizGuidelines = useMemo(() => {
    ...
    return fromRules.length ? fromRules : ["Dates and schedules will be informed in advance.", "Online rounds will be conducted via Zoom breakout rooms.", "Teams must ensure a stable internet connection during online rounds.", "Any form of malpractice or external assistance will result in disqualification.", "Decisions made by the judges are final."];
  }, [rulesArr]);

  const quizJudgingNotes = useMemo(() => {
    const fromRules = rulesArr.filter((r) => /^Judging:/i.test(r)).map(strip);
    return fromRules.length ? fromRules : ["Round 1 scores decide who moves to the semi-final.", "Semi-final winners advance to the grand finale.", "Judges’ decisions will be based on accuracy, speed, and teamwork."];
  }, [rulesArr]);
  ...
  const motionProps: MotionProps =
    prefersReducedMotion
      ? { initial: false, animate: { opacity: 1, y: 0, scale: 1 }, exit: { opacity: 1, y: 0, scale: 1 }, transition: { duration: 0 } }
        ...
        <div className="absolute inset-x-0 top-0 z-20">
          <div className="mx-auto flex max-w-6xl items-center justify-between gap-3 px-6 pt-14">
            <div className="flex items-center gap-3">
              <Link href="/#categories" scroll className="group relative inline-flex items-center gap-2 rounded-full bg-black/55 px-3 py-2 text-m text-white ring-1 ring-white/20 hover:ring-[var(--acc)]/60 focus:outline-none active:scale-[.98] transition overflow-hidden isolate" aria-label="Back to Categories">
                <span aria-hidden className="pointer-events-none absolute inset-px rounded-full z-0 [background:linear-gradient(115deg,transparent,rgba(255,255,255,.28),transparent)] opacity-0 group-hover:opacity-100 transition-opacity animate-[btnShine_3200ms_linear_infinite] motion-reduce:animate-none" />
              ...
              <span className="hidden sm:inline-flex items-center gap-2 rounded-full bg-black/45 px-3 py-2 text-m text-white ring-1 ring-white/15">
                <span className="text-base leading-none">{EMOJI_BY_SLUG[t.slug] ?? "✨"}</span>
        ...
        <div className="sticky top-16 z-10 -mx-2 overflow-x-auto px-2">
          <div className="inline-flex gap-2 rounded-2xl bg-black/50 ring-1 ring-white/12 p-2 backdrop-blur">
            {tabs.map((x) => {
              ...
              return (
                <button key={x.key} onClick={() => setTab(x.key)} className={`relative inline-flex items-center gap-2 rounded-xl px-4 py-2 text-sm transition ${active ? "text-white" : "text-white/75 hover:text-white"}`}>
